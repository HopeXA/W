...
user = None  # Will be initialized when ready
...
def get_server_settings(guild_id,channel_id):
    ...
    settings_hope_prefix = "$"  # Default prefix if not found
...
def get_snipe_time(channel,rolled,message):
    ...
    if r > 2 and user is not None and user['id'] in wished_for:
...
def on_message(resp):
    ...
    if resp.event.guild_application_commands_updated:
        ...
        slashget = None  # Initialize to avoid unbound variable
        ...
        if settings['slash_rolling'].lower().strip() == "true" and slashget is not None:
